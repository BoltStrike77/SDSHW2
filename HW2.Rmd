...
## Plot C
```{r, fig.width=6, fig.height = 5}
bikes %>% filter(hr == 9) %>% group_by(hr, workingday, weathersit) %>% summarize(average = mean(total), .groups = "drop_last") %>% ggplot() + geom_col((aes(x=weathersit, y=average)), fill = 'skyblue', col='black') + labs(x="Weather situation by intensity (1-4)", y="Average total bike rentals", title = "Average total bike rentals at 9am by weather") + facet_wrap(~workingday, labeller = as_labeller(c('0' = "Non-working day",'1' = "Working day")))
```
